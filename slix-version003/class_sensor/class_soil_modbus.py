...
class SensorSoilMoistureTemp:
    def __init__(self, port="/dev/ttyS2", slave_address=1, baudrate=9600):
        ...
        self.modbus = Modbus_Film69()
        self.modbus = Modbus_Film69(port=port, slaveaddress=slave_address, baudrate=baudrate)

    def read_data(self, addr=None):
        """
        อ่านค่าอุณหภูมิและความชื้นจาก sensor
        คำสั่ง: 01 03 00 00 00 02
        ตอบกลับ: 01 03 04 [Temp_H][Temp_L] [Moist_H][Moist_L] CRC
        """
        try:
    ...
    def set_address(self, new_address):
        """
        ตั้งค่า slave address ใหม่ (ต้องรีสตาร์ท sensor จึงจะมีผล)
        ตัวอย่าง: 01 06 02 00 00 02 -> set to 0x02
        """
