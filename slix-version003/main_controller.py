...
class SmartFarmController:
    def __init__(self):
        ...
        self.sensors_config = {}
        for port, s_info in config_data.get("sensors", {}).items():
            if s_info.get("enabled"):
                self.sensors_config[port] = s_info
    ...
    def _init_sensors(self):
        for port, s_info in self.sensors_config.items():
        ...
        while self.running:
            ...
            for port, s_info in self.active_sensors.items():
                ...
                with self.data_lock:
                    ...
                    if len(self.sensor_buffers[port]) > self.buffer_size:
                        self.sensor_buffers[port].pop(0)

                    #Store data from rain tip sensor
                ...
                if data:
                    ...
                    if not is_physically_connected:
                        ...
                        other_ports = [p for p in connected_ports if p != port_num]
                    ...
                    print(f"Port {port} ({s_type}) Read Failed / No Response (None appended to buffer)")
    ...
    def send_telemetry_data(self, port, sensor_state, s_info):
        ...
        for item in reversed(port_buffer):
            if item is not None:
                latest_valid_data = item
                break
        
        has_data = latest_valid_data is not None
        s_type = s_info['type']
        ...
        if physically_connected and has_data:
            ...
            include_data = True
            if s_type == "rainfall": #First step check rainfall value before to pack data send TB.
                data_to_send = dict(latest_valid_data)
                with self.data_lock:
                    acc_rain = self.rain_accumulators.get(port, 0.0)
                    data_to_send["rainfall"] = round(acc_rain, 2)
                    self.rain_accumulators[port] = 0.0 
            else:
                data_to_send = latest_valid_data
        ...
        if messages:
            ...
            if ok:
                print(f"[ThingsBoard] Sent Telemetry for Port {port} (Op: {operation_status}, Cur: {current_status}, Data Included: {include_data})")
    ...
    def stop(self):
        ...
        try:
            if hasattr(self, 't_sensor') and self.t_sensor.is_alive():
                self.t_sensor.join(timeout=3.0)
            if hasattr(self, 't_telemetry') and self.t_telemetry.is_alive():
                self.t_telemetry.join(timeout=3.0)
            if hasattr(self, 't_net') and self.t_net.is_alive():
                self.t_net.join(timeout=3.0)
        except Exception as e:
