...
class SensorControlSystem:
    ...
    def check_overcurrent(self):
        if self.mcp1_ready:
            for i in range(4):
                port = i + 1
                is_fault = (self.mcp1.read_pin('B', 4+i) == 0)
                self._handle_fault(port, is_fault)
                
            for i in range(4):
                port = i + 5
                is_fault = (self.mcp1.read_pin('A', 7-i) == 0)
                self._handle_fault(port, is_fault)

        if self.mcp2_ready:
            for i in range(4):
                port = i + 9
                is_fault = (self.mcp2.read_pin('B', 4+i) == 0)
                self._handle_fault(port, is_fault)
